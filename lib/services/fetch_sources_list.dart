...
Future<void> _updateSource(
  ...
  if (source.sourceCodeLanguage == SourceCodeLanguage.mihon) {
    ...
    try {
      final rawFilterList =
          await getIsolateService.get<Map<String, dynamic>>(
        source: source,
        serviceType: 'getFilterList',
      );
      filterList = FilterList.fromJson(rawFilterList);
    } catch (e) {
      AppLogger.log(
        'getFilterList failed for "${source.name}" (non-fatal): $e',
        logLevel: LogLevel.warning,
        tag: LogTag.extension_,
      );
    }
