...
class TransferServer {
  ...
  final _offerController = StreamController<IncomingOffer>.broadcast();
  Stream<IncomingOffer> get offerStream => _offerController.stream;
  ...
  Future<int> start() async {
    ...
    _port = _server!.port;
    debugPrint('[TransferServer] port=$_port');
    return _port;
  }
  ...
  Future<Response> _offer(Request req) async {
    try {
      ...
      if (!_offerController.isClosed) {
        _offerController.add(IncomingOffer(offer, completer));
      }
  ...
  void dispose() {
    stop();
    if (!_offerController.isClosed) _offerController.close();
  }
}
