...
class TransferDiscovery {
  ...
  Future<String?> getLocalIp() async {
    ...
    try {
      ...
      for (final iface in interfaces) {
        ...
        if (n.startsWith('wlan') || n.startsWith('en') || n.startsWith('wifi') || n.startsWith('ap')) {
  ...
  Future<void> start() async {
    ...
    _broadcast();
    _staleTimer = Timer.periodic(const Duration(seconds: 5), (_) => _evictStale());
  }
  ...
  void _handleDatagram(Datagram dg) {
    try {
      ...
      if (!existed && !_peersController.isClosed) {
