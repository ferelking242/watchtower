import 'dart:async';
  ...
  ///
  /// When [onStuck] is provided it is called once, right after the STUCK
  /// log entry, so the caller can auto-cancel the blocked download instead
  ...
  class DownloadStuckWatchdog {
    ...
    /// Optional callback invoked exactly once when the download is first
    /// detected as stuck. Use this to auto-cancel the task.
        ...
        if (p != null && p != _lastPercent && p >= 0) {
          ...
          if (since >= threshold && !_stuckLogged) {
            ...
            AppLogger.log(
              ...
              'last percent=$_lastPercent) — auto-cancel triggered',
              logLevel: LogLevel.warning,
              tag: LogTag.download,
            );
            // Notify the caller so it can cancel the task instead of
