...
class _PlayStoreMarketplaceViewState extends State<_PlayStoreMarketplaceView> {
  ...
  List<_ExtEntry> get _visible {
    ...
    if (_tab == 1) {
      list = list.where((entry) => _playRating(entry) >= 4.5).toList();
    } else if (_tab == 2) {
      list = list.where((entry) => _playRating(entry) >= 4.0).toList();
    }
    list.sort((a, b) {
            ...
            if (widget.loading && widget.entries.isEmpty)
              ...
              SliverToBoxAdapter(
                child: _buildSectionTitle(
                  _tab == 1 ? 'Meilleurs classements' : 'Recommandé pour vous',
                  showArrow: true,
                ),
              ),
  ...
  Widget _buildTabs() {
    const labels = ['Pour vous', 'Classements', 'Enfants', 'Catégories'];
    return SizedBox(
      height: 48,
          ...
          child: Column(
            ...
            children: [
              Text(
                labels[index],
                style: TextStyle(
                  color: _tab == index ? _green : _muted,
                  fontSize: 13,
      ...
      child: PageView.builder(
        ...
        itemBuilder: (_, index) {
          ...
          final colors = [
            ...
            const [Color(0xFF5B286B), Color(0xFF25172D)],
            const [Color(0xFF1C5672), Color(0xFF172934)],
          ][index % 3];
          return Padding(
            padding: const EdgeInsets.fromLTRB(0, 14, 10, 8),
            child: GestureDetector(
                ...
                child: Stack(
                  children: [
                    Positioned(
                      right: 18,
                      top: 18,
                      child: _PlayStoreIcon(entry: entry, size: 92),
                    ),
                    Padding(
                      padding: const EdgeInsets.fromLTRB(20, 20, 120, 16),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        mainAxisAlignment: MainAxisAlignment.end,
                        children: [
    ...
    if (entries.isEmpty) {
      return const SliverToBoxAdapter(
        child: Padding(
          ...
          child: Text(
            'Aucune extension ne correspond à cette sélection.',
  ...
  Widget _buildCategories({bool compact = false}) {
    final categories = [
      (
        Icons.auto_stories_rounded,
        'Manga',
        const Color(0xFFEF6C92),
        ItemType.manga,
      ),
      (Icons.live_tv_rounded, 'Anime', const Color(0xFF65C58A), ItemType.anime),
      (
        Icons.menu_book_rounded,
        'Romans',
        const Color(0xFF63B8DA),
        ItemType.novel,
      ),
      (
        Icons.music_note_rounded,
        'Musique',
        const Color(0xFFE6B65C),
        ItemType.music,
      ),
      (
        Icons.sports_esports_rounded,
        'Jeux',
        const Color(0xFFC388EF),
        ItemType.game,
      ),
      (Icons.language_rounded, 'Multilingue', const Color(0xFFE88768), null),
    ];
    return Padding(
...
double _playRating(_ExtEntry entry) {
  const curated = <String, double>{
    'MangaDex': 4.8,
    'Webtoons': 4.7,
    'Comick': 4.6,
    'AnimeThemes': 4.7,
  ...
  final rating = curated[entry.name] ?? entry.rating;
  return rating.clamp(3.7, 5.0);
}

int _playReviewCount(_ExtEntry entry) {
  const curated = <String, int>{
    'MangaDex': 1840,
    'Webtoons': 1260,
    'Comick': 980,
    'AnimeThemes': 720,
  ...
  return curated[entry.name] ?? entry.reviewCount;
}
...
String _playSize(_ExtEntry entry) {
  ...
  final estimatedKb = 24 + (entry.id.abs() % 112);
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 3),
                  Text(
                    '${_typeLabel(entry.contentType)} · ${entry.lang.toUpperCase()}',
                    style: const TextStyle(
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      ...
      child: Padding(
        ...
        child: Text(
          label,
          style: const TextStyle(color: Color(0xFFD6D5DA), fontSize: 11),
        ),
      ),
    );
  }
