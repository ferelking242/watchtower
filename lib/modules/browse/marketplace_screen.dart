...
String? _mktIconUrl(String? raw) {
  ...
  final m = RegExp(
    r'^https?://cdn\.simpleicons\.org/([a-z0-9-]+)$',
  ).firstMatch(raw);
  if (m != null) {
...
int? _parseMktSize(dynamic raw) {
  ...
  final match = RegExp(
    r'^\s*([\d.,]+)\s*(kb|kib|mb|mib|gb|gib)?\s*$',
    caseSensitive: false,
  ).firstMatch(raw);
...
class _MarketplaceScreenState extends ConsumerState<MarketplaceScreen>
  ...
  PluginConfiguration? _musicPluginFor(_ExtEntry entry) {
    ...
    final urls =
        <String>{entry.repoUrl, if (entry.upstream.isNotEmpty) entry.upstream}
  ...
  Future<void> _uninstall(_ExtEntry entry) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          "L'extension sera désinstallée. Réinstallable à tout moment.",
        ),
        actions: [
          TextButton(
    ...
    try {
      if (entry.contentType == ItemType.music) {
        ...
        await ref.read(metadataPluginsProvider.notifier).removePlugin(plugin);
        ref.invalidate(metadataPluginsProvider);
      } else {
        final source = await isar.sources.get(entry.id);
        if (source == null) {
          throw StateError('Extension introuvable dans le stockage local.');
        }
        final prefsIds = await isar.sourcePreferences
            .filter()
            .sourceIdEqualTo(source.id!)
            .findAll()
            .then((items) => items.map((item) => item.id!).toList());
        final prefsStringIds = await isar.sourcePreferenceStringValues
            .filter()
            .sourceIdEqualTo(source.id!)
            .findAll()
            .then((items) => items.map((item) => item.id).toList());
        await isar.writeTxn(() async {
          ...
          await isar.sourcePreferences.deleteAll(prefsIds);
          await isar.sourcePreferenceStringValues.deleteAll(prefsStringIds);
        });
  ...
  List<_ExtEntry> _forTabRaw(int tab) {
    switch (tab) {
      case _kTabManga:
        return _all.where((e) => e.contentType == ItemType.manga).toList();
      case _kTabAnime:
        return _all.where((e) => e.contentType == ItemType.anime).toList();
      case _kTabNovel:
        return _all.where((e) => e.contentType == ItemType.novel).toList();
      case _kTabGames:
        return _all.where((e) => e.contentType == ItemType.game).toList();
      case _kTabMusic:
        return _all.where((e) => e.contentType == ItemType.music).toList();
      case _kTabBinary:
        return [];
      case _kTabTools:
        return [];
  ...
  List<_ExtEntry> get _featured =>
      _all.where((e) => _kFeaturedNames.contains(e.name)).toList()
  ...
  static String _langCode(String lang) {
    ...
    if (l.contains('-'))
      return l.split('-').map((p) => p.toUpperCase()).join('-');
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<MetadataPluginState>>(metadataPluginsProvider, (
      prev,
      next,
    ) {
  ...
  Future<void> _massInstall({
    required String lang,
    SourceCodeLanguage? compat,
  }) async {
    var toInstall = _all
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String?>(
                        ...
                        items: [
                          ...
                          ...repos.map((r) {
                            final label = r.contains('ferelking242')
                                ? '⭐ Official'
                                : r.split('/').lastOrNull ?? r;
                  ...
                  child: Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<SourceCodeLanguage?>(
                        ...
                        onChanged: (v) =>
                            setState(() => _globalProgLangFilter = v),
                      ),
                    ),
                  ),
                ),
  ...
  Widget _buildError(ColorScheme cs) => Center(
    child: Column(
      ...
      children: [
        ...
        FilledButton.tonal(
          onPressed: () => _loadAll(bypassCache: true),
        ...
        children: [
          ...
          if (onSeeAll != null)
            TextButton(
              ...
              child: Text(
                'Voir tout',
                style: TextStyle(fontSize: 11.5, color: color ?? cs.primary),
              ),
            ),
        ],
      ),
  ...
  Widget buildBanner(List<_ExtEntry> entries, ColorScheme cs) {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => _startBannerTimer(show.length),
    );
    return SizedBox(
      height: 170,
              ...
              child: _searchQuery.isEmpty
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            'Essaye un autre nom ou une langue',
                            style: TextStyle(
                              fontSize: 12,
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (ctx, i) => _PlayStoreCard(
                        ...
                        hasUpdate: _hasUpdate(
                          _searchResults[i].id,
                          _searchResults[i].version,
                        ),
                        busy: _busy[_searchResults[i].id] == true,
                        onInstall: () => _install(_searchResults[i]),
                        onSettings: _installed.contains(_searchResults[i].id)
                            ? () => _openSettings(_searchResults[i].id)
                            : null,
                        onUninstall: _installed.contains(_searchResults[i].id)
                            ? () => _uninstall(_searchResults[i])
                            : null,
                      ),
                    ),
            ),
                        ...
                        Expanded(
                          child: _SearchCategoryTile(
                            ...
                            onTap: () {
                              _closeSearch();
                              _tabCtrl.animateTo(
                                _tabConstToVisual(categories[i].$1),
                              );
                            },
                          ),
                        ),
                          ...
                          Expanded(
                            child: _SearchCategoryTile(
                              ...
                              onTap: () {
                                _closeSearch();
                                _tabCtrl.animateTo(
                                  _tabConstToVisual(categories[i + 1].$1),
                                );
                              },
                            ),
                          )
  ...
  void _showMusicPluginConfigSheet(PluginConfiguration config) {
    ...
    final isDefaultMetadata =
        pluginsData.defaultMetadataPluginConfig?.slug == config.slug;
    final isDefaultAudioSource =
        pluginsData.defaultAudioSourcePluginConfig?.slug == config.slug;
    showModalBottomSheet(
      context: context,
      shape: const RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(20)),
              ...
              if (config.abilities.contains(PluginAbilities.audioSource))
                ListTile(
                  ...
                  subtitle: Text(
                    isDefaultAudioSource
                        ? 'Actuellement par défaut'
                        : 'Définir comme source audio par défaut',
                    style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant),
                  ),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          return _IconChip(
            ...
            active: active,
            onTap: () =>
                setState(() => _compatF[tab] = active ? _CompatF.all : f),
          );
        },
      ),
    );
  ...
  void _showProgLangMenu(int tab, List<SourceCodeLanguage> progLangs) {
    showModalBottomSheet(
      ...
      builder: (_) => _SimplePickerSheet(
        ...
        items: progLangs
            .map((p) => (p.index.toString(), _compatLabel(p)))
            .toList(),
        selected: _progLangFilter[tab]?.index.toString(),
        onPick: (v) {
          setState(
            () => _progLangFilter[tab] = v == null
                ? null
                : SourceCodeLanguage.values[int.parse(v)],
          );
          Navigator.pop(context);
        },
      ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Installe tout les extensions d\'un repo',
                          style: TextStyle(
                            fontSize: 11.5,
                            color: cs.onSurfaceVariant,
                          ),
            ...
            if (updatableEntries.isNotEmpty)
              SliverToBoxAdapter(
                child: _UpdatesSection(
                  ...
                  onToggle: () =>
                      setState(() => _updatesExpanded = !_updatesExpanded),
                  cs: cs,
                ),
              ),
            ...
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (ctx, i) => _PlayStoreCard(
                  ...
                  onSettings: state._installed.contains(entries[i].id)
                      ? () => state._openSettings(entries[i].id)
                      : null,
                  onUninstall: state._installed.contains(entries[i].id)
                      ? () => state._uninstall(entries[i])
                      : null,
                ),
          ...
          AnimatedCrossFade(
            ...
            secondChild: Column(
              children: [
                ...
                ...entries.map(
                  (e) => _UpdateRow(entry: e, state: state, cs: cs),
                ),
                const SizedBox(height: 4),
              ],
            ),
...
class _MassInstallSheetState extends State<_MassInstallSheet> {
  ...
  List<_ExtEntry> get _toInstall {
    ...
    var list = widget.state._all
        .where(
          (e) => e.lang == _selectedLang && !widget.installedIds.contains(e.id),
        )
        .toList();
      ...
      builder: (_, ctrl) => Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Installe toutes les extensions d\'une langue ou d\'un dépôt.',
              style: TextStyle(fontSize: 13, color: cs.onSurfaceVariant),
            ),
            const SizedBox(height: 20),
            ...
            DropdownButtonFormField<String>(
              ...
              items: widget.langs.map((l) {
                final count = widget.state._all
                    .where(
                      (e) => e.lang == l && !widget.installedIds.contains(e.id),
                    )
                    .length;
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: compatItems.map<Widget>((
                ...
                return GestureDetector(
                  onTap: _running
                      ? null
                      : () => setState(() => _selectedCompat = compat),
                  child: AnimatedContainer(
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                onPressed: (_running || toInstall.isEmpty) ? null : _doInstall,
              ),
            ),
          ],
        ),
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                ...entry.subCategories
                    .take(2)
                    .map((c) => _TagChip(label: c, cs: cs, color: cs.primary)),
...
class _MusicPluginCardState extends ConsumerState<_MusicPluginCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final isInstalled =
        plugins.asData?.value.plugins.any(
          (p) => p.repository == _repo.repoUrl,
        ) ??
        false;
    ...
    final topics = _repo.topics
        .where(
          (t) =>
              t == 'spotube-metadata-plugin' ||
              t == 'spotube-audio-source-plugin',
        )
        .map(_topicLabel)
        .toList();
          ...
          children: [
                    ...
                    onTap: () async {
                      ...
                      try {
                        ...
                        for (int _attempt = 0; _attempt < 5; _attempt++) {
                          ...
                          if (_cp != null) {
                            if (pluginConfig.abilities.contains(
                              PluginAbilities.metadata,
                            )) {
                              try {
                                await pluginsNotifier.setDefaultMetadataPlugin(
                                  pluginConfig,
                                );
                              } catch (_) {}
                            }
                            if (pluginConfig.abilities.contains(
                              ...
                              try {
                                await pluginsNotifier
                                    .setDefaultAudioSourcePlugin(pluginConfig);
                              } catch (_) {}
                            }
                            break;
                          }
            ...
            Wrap(
              ...
              children: [
                ...
                ...topics.map((t) => _TagChip(label: t, cs: cs)),
              ],
            ),
  ...
  void _showPluginConfigSheet(
    ...
    final isDefaultAudioSource =
        plugins.asData?.value.defaultAudioSourcePluginConfig?.slug ==
        config.slug;
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [
                      ...
                      compatColor.withValues(alpha: 0.65),
                    ],
                    begin: Alignment.topLeft,
                    end: Alignment.bottomRight,
                  ),
                        ...
                        Row(
                          children: [
                            Container(
                              ...
                              child: Text(
                                _MarketplaceScreenState._compatLabel(
                                  entry.compat,
                                ),
                                style: const TextStyle(
                                  fontSize: 9,
                            ...
                            Container(
                              ...
                              child: Text(
                                _MarketplaceScreenState._langCode(entry.lang),
                                style: const TextStyle(
                                  fontSize: 9,
                                  fontWeight: FontWeight.w800,
                                  color: Colors.white,
        ...
        child: iconUrl != null && iconUrl!.isNotEmpty
            ? _isSvg
                  ? SvgPicture.network(
                      ...
                      placeholderBuilder: (_) => _SkeletonShimmer(
                        width: size,
                        height: size,
                        radius: 12,
                      ),
...
class _RepoCarouselState extends State<_RepoCarousel> {
  ...
  static const _repos = [
    _RepoInfo(
      name: 'Watchtower',
      description:
          'Extensions officielles Watchtower — anime, manga, novels, musique et jeux',
  ...
  Widget _buildRepoCard(ColorScheme cs, _RepoInfo r) {
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: r.color.withValues(alpha: 0.30),
            blurRadius: 12,
            offset: const Offset(0, 4),
          ),
        ],
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _LightSettingsTile(
                    ...
                    value: showNsfw,
                    onChanged: (v) =>
                        ref.read(showNSFWStateProvider.notifier).set(v),
                  ),
                  Divider(
                    height: 1,
                    indent: 20,
...
class _AccountDropdownOverlay extends StatefulWidget {
  ...
  @override
  State<_AccountDropdownOverlay> createState() =>
      _AccountDropdownOverlayState();
}
                    ...
                    child: Center(
                      child: isAll
                          ...
                          : Column(
                              ...
                              children: [
                                Text(
                                  _MarketplaceScreenState._langFlag(l),
                                  style: const TextStyle(fontSize: 18),
                                ),
                                const SizedBox(height: 1),
                                Text(
                                  _MarketplaceScreenState._langDisplayName(l),
                                  style: TextStyle(
                                    fontSize: 10,
                                    fontWeight: FontWeight.w600,
                                    color: sel
...
class _ActiveFiltersSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (repo != null)
      chips.add((
        _MarketplaceScreenState._repoShortLabel(repo),
        () {
    ...
    if (lang != null)
      chips.add((
        _MarketplaceScreenState._langCode(lang),
        () {
    ...
    if (prog != null)
      chips.add((
        _MarketplaceScreenState._compatLabel(prog),
        () {
          state._progLangFilter.remove(tab);
          onClear();
        },
      ));
    if (state._installedOnly)
      chips.add((
        'Installées',
        () {
  ...
  Widget _skeletonCard() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: widget.cs.outlineVariant.withValues(alpha: 0.25),
        ),
      ),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.center,
...
class _BulkInstallSheetState extends State<_BulkInstallSheet> {
  ...
  List<_ExtEntry> get _toInstall {
    var list = widget.state._all
        ...
        .toList();
    if (_selLangs.isNotEmpty)
      list = list.where((e) => _selLangs.contains(e.lang)).toList();
    if (_selTypes.isNotEmpty)
      list = list.where((e) => _selTypes.contains(e.contentType)).toList();
    return list;
  }
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: _typeItems.map<Widget>((
                      (ItemType, IconData, String) item,
                    ) {
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        _selLangs.isEmpty
                            ? '(toutes)'
                            : '(${_selLangs.length} sélectionnées)',
                        style: TextStyle(
                          fontSize: 12,
                          color: cs.onSurfaceVariant,
                        ),
...
class _WTToastState extends State<_WTToast>
    with SingleTickerProviderStateMixin {
  late final AnimationController _ctrl;
  late final Animation<Offset> _slide;
  late final Animation<double> _fade;
...
class _PlayStoreMarketplaceView extends StatefulWidget {
  ...
  @override
  State<_PlayStoreMarketplaceView> createState() =>
      _PlayStoreMarketplaceViewState();
}
...
class _PlayStoreMarketplaceViewState extends State<_PlayStoreMarketplaceView> {
  ...
  List<_ExtEntry> get _featured {
    ...
    for (final entry in _visible) {
      if (names.contains(entry.name)) preferred.add(entry);
    }
  ...
  List<String> get _popularSearches {
    final types = <ItemType, String>{
      ItemType.anime: 'Watch',
      ItemType.manga: 'Manga',
      ItemType.novel: 'Romans',
      ItemType.music: 'Musique',
      ItemType.game: 'Jeux',
    };
      ...
      child: Row(
        children: [
          Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: _searchFocused || _searchController.text.isNotEmpty
          ...
          if (_searching) ...[
            ...
            TextButton(
              onPressed: _closeSearch,
              child: const Text('Annuler', style: TextStyle(color: _green)),
            ),
          ],
        ],
      ),
  ...
  Widget _buildHeroFor(List<_ExtEntry> sourceEntries) {
    final entries = sourceEntries.take(6).toList();
  ...
  Widget _buildLanguageExplorer() {
    final languages = _catalogueLanguages;
          ...
          return InkWell(
            ...
            child: Ink(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    _MarketplaceScreenState._langFlag(language),
                    style: const TextStyle(fontSize: 25),
                  ),
                  const SizedBox(height: 5),
                  Text(
                    _MarketplaceScreenState._langDisplayName(language),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: const TextStyle(
                      color: Colors.white,
    ...
    if (entries.isEmpty) {
      return const SliverToBoxAdapter(
        child: Padding(
          ...
          child: Text(
            'Aucune extension ne correspond à cette sélection. Vérifie les filtres ou actualise le catalogue.',
  ...
  Widget _buildCategories({bool compact = false}) {
    final categories = [
          ...
          return InkWell(
            ...
            child: DecoratedBox(
              ...
              child: Row(
                children: [
                  const SizedBox(width: 12),
                  Icon(category.$1, color: category.$3, size: 27),
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: Row(
              children: [
                IconButton(
                  onPressed: () => setState(() => _accountOpen = false),
                  icon: const Icon(
        ...
        SliverToBoxAdapter(
          child: Column(
            children: [
              ...
              const SizedBox(height: 4),
              Text(
                '${widget.installed.length} extension${widget.installed.length == 1 ? '' : 's'} installée${widget.installed.length == 1 ? '' : 's'}',
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                _AccountOption(
                  icon: Icons.download_done_rounded,
                  label: 'Gérer les extensions installées',
                  onTap: () => setState(() {
                    _accountOpen = false;
                    _tab = 1;
                  }),
  ...
  void _showComingSoon(String label) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('$label sera disponible prochainement.'),
        behavior: SnackBarBehavior.floating,
      ),
    );
  }
...
String _playDescription(_ExtEntry entry) {
  if (entry.description.trim().isNotEmpty) return entry.description.trim();
  final language = entry.lang == 'fr'
      ? 'français'
      : entry.lang == 'en'
      ? 'anglais'
      : entry.lang == 'multi' || entry.lang == 'all'
      ? 'multilingue'
      : entry.lang.toUpperCase();
  final kind = switch (entry.contentType) {
    ItemType.anime => 'anime et séries',
    ItemType.manga => 'mangas et bandes dessinées',
    ItemType.novel => 'romans et light novels',
    ItemType.music => 'musique et audio',
    _ => 'contenus',
  };
  return 'Source $language pour découvrir $kind dans Watchtower.';
}

String _playTypeLabel(_ExtEntry entry) => switch (entry.contentType) {
  ItemType.anime => 'Watch',
  ItemType.manga => 'Manga',
  ItemType.novel => 'Romans',
  ItemType.music => 'Musique',
...
double _playRating(_ExtEntry entry) => entry.rating.clamp(0.0, 5.0);

int _playReviewCount(_ExtEntry entry) => entry.reviewCount;

String _playSize(_ExtEntry entry) {
  if (entry.sizeBytes != null && entry.sizeBytes! > 0) {
    return _formatMktSize(entry.sizeBytes);
  }
  return 'Taille inconnue';
}

String _playRatingLabel(_ExtEntry entry) =>
    _playRating(entry) > 0 ? _playRating(entry).toStringAsFixed(1) : '—';

String _playReviewLabel(_ExtEntry entry) => _playReviewCount(entry) > 0
    ? '${_playReviewCount(entry)} avis'
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.center,
          children: [
            _PlayStoreIcon(entry: entry, size: 54),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          '${_playSize(entry)} · v${entry.version}',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                            color: _PlayStoreMarketplaceViewState._muted,
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Icon(icon, color: _PlayStoreMarketplaceViewState._muted, size: 18),
            const SizedBox(width: 12),
            Expanded(
              child: Text(
                label,
        ...
        child: Column(
          ...
          children: [
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _PlayStoreIcon(entry: entry, size: 82),
                const SizedBox(width: 16),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Column(
              children: [
                _DetailInfoRow(
                  label: 'Version',
                  value: installed && installedVersion != null
                      ? 'v$installedVersion · disponible v${entry.version}'
                      : 'v${entry.version}',
                ),
                _DetailInfoRow(label: 'Taille', value: _playSize(entry)),
                _DetailInfoRow(
                  label: 'Langue',
                  value: entry.lang.toUpperCase(),
                ),
                _DetailInfoRow(
                  label: 'Technologie',
                  value: entry.compat == SourceCodeLanguage.javascript
                      ? 'JavaScript'
                      : 'Watchtower',
                ),
                if (entry.requiresAccount)
            ...
            Text(
              ...
              style: TextStyle(
                color: _PlayStoreMarketplaceViewState._muted.withValues(
                  alpha: 0.8,
                ),
                fontSize: 11,
              ),
...
class _AccountOption extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      leading: Icon(icon, color: _PlayStoreMarketplaceViewState._muted),
      title: Text(
        label,
        style: const TextStyle(color: Colors.white, fontSize: 14),
      ),
...
class _PlayStoreIcon extends StatelessWidget {
  final _ExtEntry entry;
  final double size;
  const _PlayStoreIcon({required this.entry, required this.size});

  @override
  Widget build(BuildContext context) {
    final url = _mktIconUrl(entry.iconUrl);
    final isSvg =
        url != null &&
        (url.toLowerCase().contains('.svg') ||
            url.toLowerCase().contains('simpleicons.org'));
    return ClipRRect(
  ...
  Widget _fallback() {
    ...
    return DecoratedBox(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            _MarketplaceScreenState._typeColor(entry.contentType),
