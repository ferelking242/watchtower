...
class LogOverlayController {
  ...
  static StreamSubscription<String>? _errorSub;

  /// Call once at app startup (e.g. in main.dart after runApp) so the overlay
  /// auto-shows whenever [AppLogger] emits an ERROR-level entry.
  static void listenForErrors() {
    _errorSub ??= AppLogger.liveStream.listen((entry) {
      if (entry.contains('][ERROR]') && !instance.isVisible) {
        instance.show();
      }
    });
  }
  ...
  static bool _volumeShortcutInstalled = false;

  /// Register a [HardwareKeyboard] handler so that pressing either volume
  /// key toggles the overlay.  Safe to call multiple times (idempotent).
  static void installVolumeShortcut() {
    if (_volumeShortcutInstalled) return;
    _volumeShortcutInstalled = true;
    HardwareKeyboard.instance.addHandler(_handleVolumeKey);
  }

  static bool _handleVolumeKey(KeyEvent event) {
    if (event is KeyDownEvent) {
      final key = event.physicalKey;
      if (key == PhysicalKeyboardKey.audioVolumeUp ||
          key == PhysicalKeyboardKey.audioVolumeDown) {
        instance.toggle();
        // Returning true consumes the key and prevents the system volume
        // from changing.  Comment-out the return if you prefer the volume
        // to still change while toggling the overlay.
        return true;
      }
    }
    return false;
