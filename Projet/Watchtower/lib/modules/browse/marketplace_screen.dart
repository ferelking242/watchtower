...
// ─── Compat filter (legacy per-tab) ──────────────────────────────────────────

enum _CompatF { all, mihon, lnreader, js }

// ─── Sort order ───────────────────────────────────────────────────────────────

enum _SortOrder { none, alpha, installedFirst }

// ─── Filter state ─────────────────────────────────────────────────────────────

class _FilterState {
  final String? repo;       // null=all, 'wt', 'keiyoushi', 'aniyomi'
  final String? lang;       // null=all, specific lang code
  ...
  const _FilterState({
    ...
    this.lang,
    this.compat,
    this.sort = _SortOrder.none,
    this.installedOnly = false,
    this.updatesOnly = false,
  });

  _FilterState copyWith({
    Object? repo = _sentinel,
    Object? lang = _sentinel,
    Object? compat = _sentinel,
    _SortOrder? sort,
    bool? installedOnly,
    bool? updatesOnly,
  }) => _FilterState(
    repo: repo == _sentinel ? this.repo : repo as String?,
    lang: lang == _sentinel ? this.lang : lang as String?,
    compat: compat == _sentinel ? this.compat : compat as SourceCodeLanguage?,
    sort: sort ?? this.sort,
    installedOnly: installedOnly ?? this.installedOnly,
    updatesOnly: updatesOnly ?? this.updatesOnly,
  );

  bool get isActive => repo != null || lang != null || compat != null ||
      sort != _SortOrder.none || installedOnly || updatesOnly;
}
...
class _MarketplaceScreenState extends ConsumerState<MarketplaceScreen>
  ...
  /// True if the entry belongs to the given repo key.
  bool _matchesRepo(_ExtEntry e, String? repo) {
    if (repo == null) return true;
    if (repo == 'wt') return e.repoUrl.contains('ferelking242');
    if (repo == 'keiyoushi') return e.repoUrl.contains('keiyoushi');
    if (repo == 'aniyomi') return e.repoUrl.contains('aniyomiorg');
    return true;
  }

  /// Apply global _filters to a list and return the filtered+sorted result.
  List<_ExtEntry> _applyFilters(List<_ExtEntry> list) {
    var out = list;
    if (_filters.repo != null) out = out.where((e) => _matchesRepo(e, _filters.repo)).toList();
    if (_filters.lang != null) out = out.where((e) => e.lang == _filters.lang).toList();
    if (_filters.compat != null) out = out.where((e) => e.compat == _filters.compat).toList();
    if (_filters.installedOnly) out = out.where((e) => _installed.contains(e.id)).toList();
    if (_filters.updatesOnly) out = out.where((e) => _hasUpdate(e.id, e.version)).toList();
    switch (_filters.sort) {
      case _SortOrder.alpha:
        out = [...out]..sort((a, b) => a.name.compareTo(b.name));
      case _SortOrder.installedFirst:
        out = [...out]..sort((a, b) {
  ...
  List<_ExtEntry> _forTab(int tab) {
    List<_ExtEntry> base;
    switch (tab) {
      case _kTabManga: base = _all.where((e) => e.contentType == ItemType.manga).toList(); break;
      case _kTabAnime: base = _all.where((e) => e.contentType == ItemType.anime).toList(); break;
      case _kTabNovel: base = _all.where((e) => e.contentType == ItemType.novel).toList(); break;
      case _kTabGames: base = _all.where((e) => e.contentType == ItemType.game).toList(); break;
      case _kTabMusic: base = _all.where((e) => e.contentType == ItemType.music).toList(); break;
      default: base = _all;
    }
  ...
  List<_ExtEntry> get _searchResults {
    ...
    final raw = _all.where((e) =>
        e.name.toLowerCase().contains(q) ||
        e.lang.toLowerCase().contains(q)).toList();
    return _applyFilters(raw).take(60).toList();
  }

  // Deduplicated language list from all entries (sorted)
  List<String> get _availableLangs =>
      _all.map((e) => e.lang).toSet().toList()..sort();

  void _resetFilters() => setState(() => _filters = const _FilterState());

  void _updateFilters(_FilterState f) => setState(() => _filters = f);
  ...
  // ── Filter bar (Play Store style) ─────────────────────────────────────────────

  Widget _buildFilterBar(ColorScheme cs, ThemeData theme) {
    final repoLabel = switch (_filters.repo) {
      'wt'        => 'Watchtower',
      'keiyoushi' => 'Keiyoushi',
      'aniyomi'   => 'Aniyomi',
      _           => 'Dépôt',
    };
    final langLabel = _filters.lang == null
        ? 'Langue'
        : _langCode(_filters.lang!);
    final compatLabel = _filters.compat == null
        ? 'Compat'
        : _compatLabel(_filters.compat!);
    final hasActive = _filters.isActive;
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            // 3 filter pills (scrollable horizontally)
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    _FilterPill(
                      key: const ValueKey('pill_repo'),
                      icon: Icons.folder_outlined,
                      label: repoLabel,
                      active: _filters.repo != null,
                      cs: cs,
                      onTap: (ctx) => _showRepoPillMenu(ctx),
                    ),
                    const SizedBox(width: 8),
                    _FilterPill(
                      key: const ValueKey('pill_lang'),
                      icon: Icons.language_rounded,
                      label: langLabel,
                      active: _filters.lang != null,
                      cs: cs,
                      onTap: (ctx) => _showLangPillMenu(ctx),
                    ),
                    const SizedBox(width: 8),
                    _FilterPill(
                      key: const ValueKey('pill_compat'),
                      icon: Icons.code_rounded,
                      label: compatLabel,
                      active: _filters.compat != null,
                      cs: cs,
                      onTap: (ctx) => _showCompatPillMenu(ctx),
                    ),
                    const SizedBox(width: 4),
                  ],
                ),
  ...
  // Pill dropdown helpers using showMenu

  Future<void> _showRepoPillMenu(BuildContext ctx) async {
    final options = <({String label, String? value})>[
      (label: 'Tous les dépôts', value: null),
      (label: 'Watchtower', value: 'wt'),
      (label: 'Keiyoushi / Mihon', value: 'keiyoushi'),
      (label: 'Aniyomi', value: 'aniyomi'),
    ];
    ...
    final result = await showMenu<String?>(
      ...
      position: RelativeRect.fromLTRB(
        ...
        pos.dx + box.size.width, pos.dy + box.size.height + 300),
      items: options.map((o) => PopupMenuItem<String?>(
        value: o.value,
        child: _PillMenuItem(
          label: o.label,
          selected: _filters.repo == o.value,
    ...
    if (!mounted) return;
    // showMenu returns null if dismissed (no selection), vs the actual value
    // We use a sentinel trick: we need to differentiate "dismissed" from "selected null"
    // Since all options are in the menu, result is always the chosen value when tapped
    // If dismissed without selection, result is null but that also means "no selection"
    // We handle this by using a wrapper, but for simplicity just update always:
    _updateFilters(_filters.copyWith(repo: result));
  }

  Future<void> _showLangPillMenu(BuildContext ctx) async {
    final langs = _availableLangs.take(20).toList();
    final options = <({String label, String? value})>[
      (label: 'Toutes les langues', value: null),
      ...langs.map((l) => (label: '${_langCode(l)} — $l', value: l)),
    ];
    final box = ctx.findRenderObject() as RenderBox;
    final pos = box.localToGlobal(Offset.zero);
    final result = await showMenu<String?>(
      ...
      position: RelativeRect.fromLTRB(
        pos.dx, pos.dy + box.size.height + 2,
        pos.dx + box.size.width + 100, pos.dy + box.size.height + 400),
      items: options.map((o) => PopupMenuItem<String?>(
        value: o.value,
        child: _PillMenuItem(
          label: o.label,
          selected: _filters.lang == o.value,
          cs: Theme.of(context).colorScheme,
        ),
      )).toList(),
      color: Theme.of(context).colorScheme.surfaceContainerHigh,
    ...
    if (!mounted) return;
    _updateFilters(_filters.copyWith(lang: result));
  }

  Future<void> _showCompatPillMenu(BuildContext ctx) async {
    final cs = Theme.of(context).colorScheme;
    final options = <({String label, SourceCodeLanguage? value})>[
      (label: 'Tous les langages', value: null),
      (label: 'JavaScript / JS', value: SourceCodeLanguage.javascript),
      (label: 'Mihon / Kotlin', value: SourceCodeLanguage.mihon),
      (label: 'Dart / Flutter', value: SourceCodeLanguage.dart),
      (label: 'LNReader', value: SourceCodeLanguage.lnreader),
    ];
    final box = ctx.findRenderObject() as RenderBox;
    final pos = box.localToGlobal(Offset.zero);
    final result = await showMenu<SourceCodeLanguage?>(
      context: context,
      position: RelativeRect.fromLTRB(
        pos.dx, pos.dy + box.size.height + 2,
        pos.dx + box.size.width + 120, pos.dy + box.size.height + 300),
      items: options.map((o) => PopupMenuItem<SourceCodeLanguage?>(
        value: o.value,
        child: _PillMenuItem(
          label: o.label,
          selected: _filters.compat == o.value,
          cs: cs,
        ),
      )).toList(),
    ...
    if (!mounted) return;
    _updateFilters(_filters.copyWith(compat: result));
  }
...
class _AdvancedFilterSheet extends StatefulWidget {
  ...
  const _AdvancedFilterSheet({required this.state, required this.initial});
...
class _AdvancedFilterSheetState extends State<_AdvancedFilterSheet> {
  ...
  static const _sortOptions = <({String label, IconData icon, _SortOrder value})>[
    (label: 'Par défaut', icon: Icons.sort_rounded, value: _SortOrder.none),
    (label: 'Alphabétique', icon: Icons.sort_by_alpha_rounded, value: _SortOrder.alpha),
    (label: 'Installées en premier', icon: Icons.download_done_rounded, value: _SortOrder.installedFirst),
  ];

  static const _repoOptions = <({String label, IconData icon, String? value})>[
    (label: 'Tous les dépôts', icon: Icons.all_inbox_rounded, value: null),
    (label: 'Watchtower', icon: Icons.folder_special_rounded, value: 'wt'),
    (label: 'Keiyoushi / Mihon', icon: Icons.android_rounded, value: 'keiyoushi'),
    (label: 'Aniyomi', icon: Icons.live_tv_rounded, value: 'aniyomi'),
  ];

  static const _compatOptions = <({String label, IconData icon, SourceCodeLanguage? value})>[
    (label: 'Tous les langages', icon: Icons.code_off_rounded, value: null),
    (label: 'JavaScript / TS', icon: Icons.javascript_rounded, value: SourceCodeLanguage.javascript),
    (label: 'Mihon / Kotlin', icon: Icons.android_rounded, value: SourceCodeLanguage.mihon),
    (label: 'Dart / Flutter', icon: Icons.flutter_dash_rounded, value: SourceCodeLanguage.dart),
    (label: 'LNReader', icon: Icons.menu_book_rounded, value: SourceCodeLanguage.lnreader),
  ];
          ...
          Flexible(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  // ── Sort ────────────────────────────────────────────────────
                  _sectionTitle(cs, 'Trier par', Icons.sort_rounded),
                  const SizedBox(height: 10),
                  Wrap(
                    spacing: 8, runSpacing: 8,
                    children: _sortOptions.map((o) {
                      final sel = _draft.sort == o.value;
                      return _ChoiceChip(
                        icon: o.icon, label: o.label, selected: sel, cs: cs,
                        onTap: () => setState(() => _draft = _draft.copyWith(sort: o.value)),
                      );
                    }).toList(),
                  ),
                  const SizedBox(height: 20),
                  // ── Display toggles ─────────────────────────────────────────
                  _sectionTitle(cs, 'Afficher', Icons.visibility_rounded),
                  const SizedBox(height: 10),
                  _ToggleRow(
                    label: 'Installées seulement',
                    icon: Icons.download_done_rounded,
                    value: _draft.installedOnly,
                    cs: cs,
                    onChanged: (v) => setState(
                        () => _draft = _draft.copyWith(installedOnly: v)),
                  ),
                  const SizedBox(height: 8),
                  _ToggleRow(
                    label: 'Avec mises à jour',
                    icon: Icons.system_update_alt_rounded,
                    value: _draft.updatesOnly,
                    cs: cs,
                    onChanged: (v) => setState(
                        () => _draft = _draft.copyWith(updatesOnly: v)),
                  ),
                  const SizedBox(height: 20),
                  // ── Repo ────────────────────────────────────────────────────
                  _sectionTitle(cs, 'Dépôt', Icons.folder_outlined),
                  const SizedBox(height: 10),
                  Wrap(
                    spacing: 8, runSpacing: 8,
                    children: _repoOptions.map((o) {
                      final sel = _draft.repo == o.value;
                      return _ChoiceChip(
                        icon: o.icon, label: o.label, selected: sel, cs: cs,
                        onTap: () => setState(() =>
                            _draft = _draft.copyWith(repo: o.value)),
                      );
                    }).toList(),
                  ),
                  const SizedBox(height: 20),
                  // ── Language Prog ────────────────────────────────────────────
                  _sectionTitle(cs, 'Langage Prog', Icons.code_rounded),
                  const SizedBox(height: 10),
                  Wrap(
                    spacing: 8, runSpacing: 8,
                    children: _compatOptions.map((o) {
                      final sel = _draft.compat == o.value;
                      return _ChoiceChip(
                        icon: o.icon, label: o.label, selected: sel, cs: cs,
                        onTap: () => setState(() =>
                            _draft = _draft.copyWith(compat: o.value)),
                      );
                    }).toList(),
                  ),
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  ...
                  child: FilledButton(
                    onPressed: () {
                      widget.state._updateFilters(_draft);
...
// ── Chip widget for AdvancedFilterSheet ────────────────────────────────────────
...
// ── Toggle row for AdvancedFilterSheet ────────────────────────────────────────
