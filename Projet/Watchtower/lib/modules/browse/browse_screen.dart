...
import 'package:watchtower/modules/plugins/plugins_screen.dart'
    show installedPluginsProvider, InstalledPlugin;
...
// ─── Plugins browse tab ────────────────────────────────────────────────────────
// Simple themed list of installed plugins — no custom dark colours.

class _PluginsBrowseTab extends ConsumerWidget {
  const _PluginsBrowseTab();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final asyncPlugins = ref.watch(installedPluginsProvider);
    ...
    return asyncPlugins.when(
      ...
      error: (e, _) => Center(
        child: Padding(
          ...
          child: Text('Erreur : $e', style: TextStyle(color: cs.error)),
        ),
      ),
      data: (plugins) {
        if (plugins.isEmpty) {
          return Center(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text('Allez dans Marketplace → Plugins',
                  style: TextStyle(
          ...
          itemBuilder: (ctx, i) {
            ...
            return ListTile(
              leading: Container(
                ...
                child: Icon(Icons.extension_rounded, color: cs.primary, size: 20),
              ),
              title: Text(
                p.id,
              ...
              subtitle: Text(
                'v${p.version} · ${p.enabled ? "Actif" : "Désactivé"}',
                style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant),
              ),
              trailing: Switch.adaptive(
                value: p.enabled,
                onChanged: (_) =>
                    ref.read(installedPluginsProvider.notifier).toggle(p.id),
              ),
            );
          },
        );
